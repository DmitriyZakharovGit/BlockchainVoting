...
async function add(query) {
    let request = query;
    let voters = query.voters.map((voter) => ({ email: voter, uuid: uuidGenerator() }));
    let candidates = query.candidates.map((candidate) => ({ candidate: candidate, uuid: uuidGenerator() }));
...
let getUuidArray = (array) => array.map((element) => element.uuid);

let mergeArray = (addresses, arrayObjects) => arrayObjects.map((object, index) => {
